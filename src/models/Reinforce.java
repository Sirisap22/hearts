    ...
    while (!anyOneDone(bot)) {
      ...
      for (int i = 0; i < 13; i++) {
        for (int j = 0; j < 4; j++) {
          ...
          System.out.print(table.getPlayedNumber() + ". " + bot[First].getName() + " played " + card.toString()
              + " Break : " + !bot[First].hasSuit(table.getFirstSuit()) + "\t");
        ...
        System.out
            .println(bot[winner].getName() + " has to keep the card and got " + (bot[winner].getPoints() - winnerScore)
                + " point, first suit : " + firstCardSuit + ", Is broken : " + table.isBroken() + "\n");
      }
